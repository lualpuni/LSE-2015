...
{
   int i;
   drone_ID_t drone_ID = 0;
   for (i = 0; i < packet.length; i++)
   {
      drone_ID = (drone_ID + char_to_hex(packet.data[i]) )*16;
   }
   drone_ID = drone_ID / 16;  //En la última iteración no hay que multiplicar por 16
   return drone_ID;
}
...
void drone_ID_to_data(drone_ID_t drone_ID, packet_t *packet)
{
   ...
   for (i = 0; i < packet->length; i++)
   {
      ...
      packet->data[packet->length - i - 1] = hex_to_char(remainder);
